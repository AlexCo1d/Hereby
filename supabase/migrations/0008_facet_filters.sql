-- 0008 — parity: skill-level / group-size facet filters in Discover.
--
-- The client now sends skillLevels[], minSeats, maxSeats in the DiscoverFilter
-- JSON (see FilterSheet.facetsToFilter + services/mock/index.ts applyFilter).
-- The mock backend already honors them; port the same forgiving semantics to
-- list_posts_for_viewer so both backends filter identically:
--   • skillLevels: a post passes if it has NO requirement (skill_mode null or
--     'any') OR its skill_level is in the selected set.
--   • minSeats / maxSeats: inclusive seat bounds (seats defaults to 1).
--
-- Only the parsing block and three WHERE predicates are added; everything else
-- is unchanged from 0007.
